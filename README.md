...
- **Per-topic practice + mid-progress state (desktop & phone parity)** — the deck
  ships as **28 per-topic subdecks** under "GMAT Focus" (studying the parent is
  the full exam), and it **bakes in synthetic review history** so a fresh install
  opens "midway through practising": the three scores populate and the coverage
  map shows real topics as practised, instead of a wiped, from-scratch deck. A
  re-runnable test — `content/scripts/topic_practice_test.py` — drives the **real
  desktop startup migration** (`anki/qt/aqt/gmat_deck_migrate.py`) against a
  legacy/flat collection and asserts: **28 populated subdecks**, the exam parent
  **expanded** (every topic visible, not one collapsed row), **all three scores
  non-abstaining** (Memory≈97 / Performance≈83 / Readiness≈580, medium), **every
  topic practiceable** (covered → due reviews, uncovered → new cards; no "session
  complete" dead-ends), and **idempotent** on a second launch. The seeded deck is
  regenerated by `content/tools/seed_demo_apkg.py`, which self-verifies that all
  three scores survive a fresh reimport (via the same Rust importer both apps use).
  Full per-score methodology: [`docs/scores/`](docs/scores/).
...
# In the app: pick any of the 28 topic decks under "GMAT Focus" (or the parent
#             for the full exam) -> Study Now; Tools -> GMAT Readiness for the
#             three scores + topic-coverage map. First launch reorganises an
#             older collection into the per-topic decks and seeds a mid-progress
#             demo state, so the scores + coverage show real progress.
```
