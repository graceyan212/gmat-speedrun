...
- **Tap-to-answer auto-grading (AI).** Just tap your choice — the shared engine sets Again/Hard/Good/Easy for you from whether you were right, how fast you answered vs the item's target time, and the item's difficulty vs your ability (θ). No manual self-rating. Computed in Rust (`grade_answer`, `GradeAnswer` RPC), so both apps grade identically; the manual buttons stay as an override. On by default on iPhone (settings toggle), opt-in on desktop (Preferences → Scheduler).
