...
- **Per-topic practice (desktop & phone parity)** — the deck ships as **28
  per-topic subdecks** under "GMAT Focus" (studying the parent is the full exam),
  so you can drill one topic or sit the whole exam. A re-runnable test —
  `content/scripts/topic_practice_test.py` — drives the **real desktop startup
  migration** (`anki/qt/aqt/gmat_deck_migrate.py`) against a legacy/flat
  collection and asserts: **28 populated subdecks**, the exam parent **expanded**
  (every topic visible in the deck list, not one collapsed row), **every topic
  practiceable** (studying a topic serves a card — no "session complete"
...
#             older collection into the per-topic decks (and expands them so all
#             28 topics are visible).
```
