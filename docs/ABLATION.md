...
| Arm | Policy | Mirrors |
|-----|--------|---------|
| **ADAPTIVE** | Pick the item whose difficulty $b$ is nearest the learner's *current* estimated ability $\hat\theta$, re-estimating $\hat\theta$ after every answer. | `adaptive.rs` — the `fit_distance` $= \lvert b - \hat\theta\rvert$ tie-break, plus `estimate_ability` (Rasch/1PL Newton solver). |
| **OFF** | The app's default *non-adaptive* order: points-at-stake, weakest-topic first ($\text{weight} \times \text{weakness}$, highest weight first). $\theta$ is still estimated for the metric but does **not** steer selection. | `sorting.rs::sort_review` / `topic_mastery::points_at_stake_weights`. |
...
- **Model (Rasch / 1PL).** Each item has difficulty `b` (logit). A learner with
  true ability $\theta_{\text{true}}$ answers correctly with $P = \sigma(\theta_{\text{true}} - b)$.
  ...
  including `SCALE = 4.0`, the $[-4, 4]$ $\theta$ clamp, and the 10-iteration
...
- **Equal budget.** Every arm runs `N = 60` item-attempts. Repeated items
  ...
  the revlog.
- **Seeds.** `200` seeds. Each seed draws one learner's $\theta_{\text{true}}$ uniformly
...
`N = 60` attempts/arm, `200` seeds, band $= \lvert b - \theta_{\text{true}}\rvert < 1$, mean ± sd:

| Arm | % items in desirable band (higher better) | $\lvert\hat\theta - \theta_{\text{true}}\rvert$ @ N (lower better) |
|-----|:---:|:---:|
...
- **Ability recovery:** ADAPTIVE's ability-estimation error is **0.212** logits
  ...
  items near $\hat\theta$ is exactly where a Rasch item is most informative
  (Fisher information peaks at $b = \theta$).
...
- **The model is the thing being assumed, then measured.** Answers are generated
  ...
  $\hat\theta$ *is* selecting into the band once $\hat\theta$ is near $\theta_{\text{true}}$.
- **Difficulty data is coarse.** Only 68 of 369 items are finely AI-rated; the
  ...
  the true deck spread on the ability axis is narrow ($b$ in $\sim[-1.4, +1.3]$). A
...
- **OFF is a faithful-but-partial proxy.** It reproduces the points-at-stake
  ...
  modeled.
- **$\hat\theta$ is not used to *grade*, only to *select* / *report*.** The band
  metric judges against $\theta_{\text{true}}$ (known only in simulation); in the app the
  learner never sees $\theta_{\text{true}}$, so the app-side benefit is the reported score
  range converging, which metric (b) stands in for.
- **Absolute numbers are seed- and parameter-dependent** (`N`, seed count,
  $\theta_{\text{true}}$ range, `SCALE`, band width). The *ordering* of the arms is the
